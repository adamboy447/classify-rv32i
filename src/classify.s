...
classify:
    ...
    # Multiply t0 * t1 without using mul instruction
    # Result will be in a0
    mv a0, x0     # Initialize a0 to 0
    mv t2, t1     # Copy t1 to t2
mul_h_size_loop:
    beq t2, x0, mul_h_size_done  # If t2 == 0, exit loop
    add a0, a0, t0               # a0 += t0
    addi t2, t2, -1              # t2 -= 1
    j mul_h_size_loop
mul_h_size_done:

    slli a0, a0, 2  # Multiply by 4 to get byte size
    jal malloc
    beq a0, x0, error_malloc
    ...
    # Calculate length of h array without using mul
    lw t0, 0(s3)    # t0 = m0_rows
    lw t1, 0(s8)    # t1 = input_cols
    mv a1, x0       # Initialize a1 to 0
    mv t2, t1       # Copy t1 to t2
mul_h_length_loop:
    beq t2, x0, mul_h_length_done
    add a1, a1, t0  # a1 += t0
    addi t2, t2, -1
    j mul_h_length_loop
mul_h_length_done:
    ...
    lw t1, 0(s8)  # t1 = h_cols (same as input_cols)

    # Multiply t0 * t1 without using mul
    mv a0, x0     # Initialize a0 to 0
    mv t2, t1     # Copy t1 to t2
mul_o_size_loop:
    beq t2, x0, mul_o_size_done
    add a0, a0, t0  # a0 += t0
    addi t2, t2, -1
    j mul_o_size_loop
mul_o_size_done:
    ...
    lw a1, 4(sp)     # Restore a1 (argument pointer)
    ...
    # Calculate length of o array without using mul
    lw t0, 0(s5)     # t0 = m1_rows
    lw t1, 0(s8)     # t1 = h_cols
    mv a1, x0        # Initialize a1 to 0
    mv t2, t1        # Copy t1 to t2
mul_o_length_loop:
    beq t2, x0, mul_o_length_done
    add a1, a1, t0   # a1 += t0
    addi t2, t2, -1
    j mul_o_length_loop
mul_o_length_done:
